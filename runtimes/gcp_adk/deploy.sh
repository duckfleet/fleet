...
# Operator address for the daily new-user digest (blank -> falls back to NOTIFY_EMAIL).
ADMIN_EMAIL=$(grep -E '^DUCKFLEET_ADMIN_EMAIL=' .env | head -1 | cut -d= -f2- || true)
...
gcloud run jobs deploy "$JOB" --source . --region "$REGION" --project "$PROJECT" \
  --set-env-vars="GOOGLE_GENAI_USE_VERTEXAI=TRUE,GOOGLE_CLOUD_PROJECT=$PROJECT,GOOGLE_CLOUD_LOCATION=global,DUCKFLEET_PROJECT_ID=$PROJECT,DUCKFLEET_REGION=$REGION,DUCKFLEET_MODEL_FAST=gemini-3.7-flash,DUCKFLEET_MODEL_STRONG=gemini-3.7-flash,DUCKFLEET_REPLAY=$REPLAY,DUCKFLEET_PROFILE_ID=$PROFILE_ID,DUCKFLEET_BIGQUERY_DATASET=duckfleet,DUCKFLEET_RESEND_FROM=$RESEND_FROM,DUCKFLEET_LIST_UNSUBSCRIBE=$LIST_UNSUB,DUCKFLEET_GMAIL_SENDER=$SENDER,DUCKFLEET_NOTIFY_EMAIL=$NOTIFY_EMAIL,DUCKFLEET_ADMIN_EMAIL=$ADMIN_EMAIL" \
